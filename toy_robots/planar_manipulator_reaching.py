...
if __name__ == '__main__':
    ...
    # Initial and final configuration (joint positions)
    theta_0 = torch.tensor([2.1*torch.pi/4,-1.5*torch.pi/4]).view(1,-1).to(device) #Initial configuration
    theta_3 = 1*theta_0 #final configuration


    # Pick and place location (via-points: x_1 and x_2),define range for x_1 and x_2
    ...
    domain_x1 = [torch.linspace(x_min_pick,x_max_pick,args.d0_x),
                torch.linspace(y_min_pick,y_max_pick,d0_y)]  # value range for x_1
    domain_x2= [torch.linspace(x_min_place,x_max_place,args.d0_x),
                torch.linspace(y_min_place,y_max_place,d0_y)]  # value range for x_2

    #define range for theta and w
